$(document).ready(() => {
    $("button").click(e => {
        ...
        $(".error").empty();
        if (!name || name.length < 4 || name.match(/ /))
            return $(".error").text("Please provide a name of at least 4 characters with no spaces.");
...
function load(res) {
    if (res.status === 502)
        return $(".error").text("Server not found.");
    if (res.status !== 200)
        return $(".error").text("There was an error.");
    res.json().then((info) => {
    ...
    Object.keys(info).forEach(i => {
        ...
        switch (i) {
            ...
            case ("plugins"):
                ...
                break;
            default:
                html = getField(i.split("_").map(w => w.charAt(0).toUpperCase().concat(w.substring(1, w.length))).join(" "), typeof info[i] === "string" ? info[i].split("_").map((w) => w.charAt(0).toUpperCase().concat(w.substring(1, w.length))).join(" ") : info[i]);
                break;
        }
        body += html;
    });
...
function getField(key, value) {
    if (typeof value === "object" && !Array.isArray(value)) {
        value = `<ul class="properties">${Object.keys(value).map(prop => {
            if (!value[prop] && typeof value[prop] !== "boolean" && typeof value[prop] !== "number")
                value[prop] = "Not provided";
